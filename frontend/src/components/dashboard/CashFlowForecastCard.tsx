  ...
  return (
    <Card className={`border-${isHighRisk ? 'orange' : 'emerald'}-200 bg-${isHighRisk ? 'orange' : 'emerald'}-50/30`}>
      <CardHeader className="pb-2">
        <CardTitle className="text-sm font-semibold flex items-center text-slate-800 dark:text-slate-200">
      ...
      <CardContent>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <p className="text-xs text-slate-500 dark:text-slate-400 uppercase font-semibold">Required Working Capital</p>
            <p className="text-2xl font-bold mt-1">
              ${forecast.required_working_capital.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
            </p>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Includes ${forecast.reorder_cost.toLocaleString()} reorder cost</p>
          </div>
          <div className="flex flex-col items-end justify-center max-w-[70%]">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">Confidence: {forecast.confidence_score}%</p>
          </div>
        </div>
      </CardContent>
    </Card>
