...
export default function SignupPage() {
  ...
  // Only length is enforced. The five-rule gate rendered five red crosses on
  // first paint and disabled the primary button before the user typed a
  // character — enterprise friction on a free-trial form. The strength meter
  // below still guides toward a stronger password.
    ...
    <AuthShell>
      <div className="w-full max-w-md bg-card border border-border rounded-2xl p-6 sm:p-8 shadow-xl space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-muted-foreground">Free for 14 days. No credit card.</p>
        </div>
        ...
        <form onSubmit={handleSignup} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-bold text-foreground uppercase tracking-wider mb-2">Full Name <span className="font-normal normal-case text-muted-foreground">(optional)</span></label>
          ...
          <div>
            ...
            {password.length > 0 && (
              <div className="mt-3 space-y-2">
                ...
                {/* Only the enforced rule is shown. Rendering five rules with
                    red crosses implied failure for a password the form now
                    accepts — the strength meter above already guides toward a
                    stronger one without blocking anyone. */}
