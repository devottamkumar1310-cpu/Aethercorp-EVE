...
export default function DocumentDetailPage() {
  ...
  const renderExtractedData = (data: any) => {
    if (!data || typeof data !== "object") {
      return (
        <div className="p-4 bg-slate-50 dark:bg-slate-900/80 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800/60 rounded-xl text-center text-xs text-slate-500 dark:text-slate-400 italic">
      ...
      <div className="space-y-4">
        {entries.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 animate-fade-in">
            {entries.map(([key, val]) => (
              <div key={key} className="bg-slate-50 dark:bg-slate-900/80 dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800/60 rounded-xl p-3.5">
                <span className="text-[10px] uppercase font-bold tracking-wider text-slate-500 dark:text-slate-400 block">{formatKey(key)}</span>
                <span className="text-sm font-bold text-slate-800 dark:text-slate-200 mt-1 block truncate">
                  {String(val)}
                </span>
              </div>
            ))}
        ...
        {arrays.map(([key, val]: any) => (
          <div key={key} className="space-y-2 pt-2 animate-fade-in">
            <span className="text-xs font-bold text-slate-600 dark:text-slate-400 block uppercase tracking-wider">{formatKey(key)}</span>
            <div className="bg-slate-50 dark:bg-slate-900/80 dark:bg-slate-950/60 border border-slate-850 rounded-xl overflow-x-auto">
              <table className="w-full text-left text-xs border-collapse min-w-[400px]">
                <thead>
                  <tr className="bg-slate-100 dark:bg-slate-900/50 dark:bg-slate-900 border-b border-slate-850 text-slate-600 dark:text-slate-400">
                ...
                <tbody>
                  {val.map((item: any, idx: number) => (
                    <tr key={idx} className="border-b border-slate-850/40 hover:bg-slate-100 dark:bg-slate-900/50 dark:bg-slate-900/20 text-slate-700 dark:text-slate-300">
      ...
      <div className="space-y-6 p-6 max-w-7xl mx-auto text-slate-950 animate-pulse">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-card border border-border rounded-2xl flex flex-col min-h-[500px]">
            ...
            <div className="flex-1 bg-slate-100 dark:bg-slate-900/50 p-4" />
          </div>
  ...
  if (!document) {
    return (
      <div className="min-h-[60vh] flex flex-col justify-center items-center text-slate-900 dark:text-slate-100">
  ...
  return (
    <div className="space-y-6 p-6 max-w-7xl mx-auto text-slate-900 dark:text-slate-100">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800/60 pb-5">
        <div className="flex items-center gap-4">
          <Link 
            ...
            className="p-2 bg-slate-200 dark:bg-slate-800 hover:bg-slate-700 text-slate-700 dark:text-slate-300 border border-slate-300 dark:border-slate-700 rounded-xl transition-all"
          >
            <ArrowLeft className="h-5 w-5" />
          </Link>
          <div>
            <h1 className="text-2xl font-bold tracking-tight max-w-md truncate">{document.filename}</h1>
            <div className="flex items-center gap-3 mt-1.5 text-xs text-slate-600 dark:text-slate-400 font-medium">
              <span className="flex items-center gap-1">
                <Clock className="h-3.5 w-3.5" />
                {new Date(document.created_at).toLocaleDateString()}
              </span>
              <span>•</span>
              <span className="px-2.5 py-0.5 bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-full font-semibold border border-slate-750">
        ...
        <div className="flex items-center gap-3">
          {previewUrl && (
            <button 
              ...
              className="flex items-center gap-2 px-4 py-2 bg-slate-200 dark:bg-slate-800 hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-semibold border border-slate-300 dark:border-slate-700 rounded-xl transition-all"
            >
          ...
          {(document.status === "completed" || document.status === "success") && (
            <Link 
              ...
              className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 hover:opacity-90 text-slate-900 dark:text-slate-100 dark:text-white font-bold rounded-xl shadow-lg transition-all"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-slate-100 dark:bg-slate-900/50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800/80 rounded-2xl overflow-hidden flex flex-col min-h-[550px]">
          <div className="p-4 bg-slate-50 dark:bg-slate-900/80 dark:bg-slate-950/40 border-b border-slate-200 dark:border-slate-800/80 flex items-center justify-between">
            <h3 className="font-semibold text-sm flex items-center gap-2 text-slate-700 dark:text-slate-300">
          ...
          <div className="flex-1 bg-slate-50 dark:bg-slate-900/80 dark:bg-slate-950/60 p-4 flex items-center justify-center relative min-h-[450px]">
            {previewUrl ? (
              document.content_type.startsWith("image/") ? (
                ...
                <div className="w-full max-w-md p-6 space-y-6 text-center animate-fade-in">
                  ...
                  <div className="space-y-2">
                    <h4 className="font-bold text-slate-800 dark:text-slate-200">Structured Dataset Ingested</h4>
              ...
              <div className="text-center p-6 space-y-3">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">Preparing preview stream...</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          {document.coo_insights ? (
            ...
            <div className="bg-slate-100 dark:bg-slate-900/50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800/80 rounded-2xl p-5 text-center text-slate-500 dark:text-slate-400 text-sm italic">
          ...
          {document.quality_assessment && (
            <div className="bg-slate-100 dark:bg-slate-900/50 backdrop-blur-md border border-slate-200 dark:border-slate-800/80 rounded-2xl p-5 shadow-xl space-y-4">
          ...
          <div className="bg-slate-100 dark:bg-slate-900/50 backdrop-blur-md border border-slate-200 dark:border-slate-800/80 rounded-2xl p-5 shadow-xl space-y-4">
            <h3 className="font-bold text-base text-slate-800 dark:text-slate-200 border-b border-slate-200 dark:border-slate-800 pb-3 flex items-center justify-between">
          ...
          {/* Processing Status */}
          <div className="bg-slate-100 dark:bg-slate-900/50 backdrop-blur-md border border-slate-200 dark:border-slate-800/80 rounded-2xl p-5 shadow-xl space-y-4">
            <h3 className="font-bold text-base text-slate-800 dark:text-slate-200 border-b border-slate-200 dark:border-slate-800 pb-3">
              Processing Status
            </h3>
 ...
 : "bg-slate-100 dark:bg-slate-900/50 dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400"
 }`}>
                        {stepStatus === "completed" ? (
